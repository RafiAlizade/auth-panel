...
function AdminRegister() {
    ...
    const [containersRepeat, setContainersRepeat] = useState(false)
    ...
    const showEyeRepeat = () => {
        const passInput = document.querySelector('.a_login__passinputrepeat')
        if (passInput.type === 'password') {
            passInput.type = 'text'
        ...
        <section className="admin__login">
            ...
            <div className="a_login__container">
                <form action="#" className="a_login__form">
                        ...
                        <label htmlFor="adminPasswordTwo">
                        <span className="a__login_label">
                                Password Repeat
                            </span>

                           <div className="a_login__password" style={ { border: containersRepeat ? '0.15rem solid rgb(209, 233, 255)' : '0.06rem solid rgb(208, 213, 221)' }}>
                                 <input type="password" placeholder='Enter password repeat' onFocus={() => setContainersRepeat(true)}  onBlur={() => setContainers(false)} className='a_login__passinputrepeat' name="adminPassword" onChange={(e) => setPasswordTwo(e.target.value)} required />
                                <button type='button' className='a_login__showpass' onClick={showEyeRepeat}>{showPasswordRepeat ? < EyeSlash /> : < Eye />}</button>
                           </div>
                        </label>
